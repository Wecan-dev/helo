...
{
	update_option('rtwwdpd_category_offer_msg', $_POST['rtwwdpd_pro_offer']);
}
if(isset($_GET['delcate']))
{
	...
	update_option('rtwwdpd_single_cat_rule',$rtwwdpd_products_option);
...
if(isset($_POST['rtwwdpd_save_cat'])){
	...
	$rtwwdpd_cat_option = get_option('rtwwdpd_single_cat_rule');
...
<div class="rtwwdpd_add_buttons">
	...
	<input class="rtw-button rtwwdpd_combi_cat" type="button" name="rtwwdpd_combi_prod_rule" value="<?php esc_attr_e( 'Add Combi Category Rule', 'rtwwdpd-woo-dynamic-pricing-discounts-with-ai' ); ?>" />
</div>
				...
				<div class="panel-wrap product_data">
					...
					<div class="panel woocommerce_options_panel">
						<div class="options_group rtwwdpd_active" id="rtwcat_rule_tab">
							<input type="hidden" id="rtw_save_single_cat" name="rtw_save_single_cat" value="<?php echo esc_attr($_GET['editcat']); ?>">
							<table class="rtwwdpd_table_edit">
								...
								<tr>
									...
									<td>
										...
										?>
										<select name="category_id" id="category_id" class="wc-enhanced-select rtw_clsscategory rtwwdpd_prod_class" data-placeholder="<?php esc_attr_e('Select category', 'rtwwdpd-woo-dynamic-pricing-discounts-with-ai'); ?>">
										<?php 
										if(is_array($cats) && !empty($cats))
										{
				            				foreach ($cats as $key => $value) {
					            				...
					            				{ 			            					
					            					if($key == $rtwwdpd_prev_prod['category_id'])
				            						{
			            								echo '<option value="' . esc_attr($key) . '"' . selected($key, $rtwwdpd_prev_prod['category_id']) . '>' . esc_html($cats[$key]) . '</option>'; 
			            							}
			            							else{
	...
	<table class="rtwtable table table-striped table-bordered dt-responsive nowrap" data-value="categor" cellspacing="0">
		...
		if(is_array($rtwwdpd_products_option) &&  !empty($rtwwdpd_products_option)) { ?>
			...
			foreach ($rtwwdpd_products_option as $key => $value) {
				...
				{
					echo esc_html__( isset( $products[$value['category_id']] ) ? $products[$value['category_id']] : '', 'rtwwdpd-woo-dynamic-pricing-discounts-with-ai' );
				}
				echo '</td>';

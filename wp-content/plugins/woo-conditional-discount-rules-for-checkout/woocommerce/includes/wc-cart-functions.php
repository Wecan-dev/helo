...
function wc_get_raw_referer() {
    ...
    $request_http_refer=filter_input(INPUT_SERVER,'HTTP_REFERER',FILTER_SANITIZE_STRING);
    $server_http_refer=filter_input(INPUT_GET,'_wp_http_referer',FILTER_SANITIZE_STRING);
...
function wc_cart_totals_coupon_html($coupon) {
    ...
    $value = implode(', ', $value) . ' <a href="' . esc_url(add_query_arg('remove_coupon', rawurlencode($coupon->code), defined('WOOCOMMERCE_CHECKOUT') ? wc_get_checkout_url() : wc_get_cart_url() )) . '" class="woocommerce-remove-coupon" data-coupon="' . esc_attr($coupon->code) . '">' . __('[Remove]', 'woocommerce') . '</a>';

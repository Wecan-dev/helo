...
<div class="cart_totals <?php if ( WC()->customer->has_calculated_shipping() ) {
	...
	<table cellspacing="0" class="shop_table shop_table_responsive">
		...
		<?php endif; ?>
			<?php $dis=WC()->cart->get_fees();
			 
			 if(!empty($dis)){
					 foreach ($dis as $key => $value) {
					 	$filter_dis[$value->name]=$value;
					 }
					 ?>
				<?php foreach ( $filter_dis as $dpad ) : ?>
					<tr class="dpad">
		...
		<tr class="loader-image-checkout" id="loader_image_checkout" style="display:none;">
			<td>
				<img src="<?php echo esc_url(WDPAD_PRO_PLUGIN_URL) . 'public/images/custom_checkout_loading.gif' ?>" />
			</td>
		</tr>

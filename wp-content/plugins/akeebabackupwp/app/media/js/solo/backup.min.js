...
akeeba.Backup.timerTick=function(){try{console.log("Timer tick")}catch(c){}var a=akeeba.System.getOptions("akeeba.Backup.maxExecutionTime",14),b=akeeba.System.getOptions("akeeba.Backup.runtimeBias",75);akeeba.Backup.resetTimeoutBar();akeeba.Backup.startTimeoutBar(a,b);akeeba.System.doAjax({ajax:"step",tag:akeeba.Backup.tag,backupid:akeeba.Backup.backupid},akeeba.Backup.onStep,akeeba.Backup.onError,!1)};
akeeba.Backup.startTimeoutBar=function(a,b){var c=0;akeeba.Backup.timeoutTimer=setInterval(function(){c++;var e=document.querySelector("#response-timer div.text");e&&(e.textContent=akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_LASTRESPONSE").replace("%s",c.toFixed(0)))},1E3)};
...
akeeba.Backup.renderBackupSteps=function(a){var b="akeeba-label--green";""==a&&(b="akeeba-label--grey");document.getElementById("backup-steps").innerHTML="";for(var c=akeeba.System.getOptions("akeeba.Backup.domains",{}),e=0;e<c.length;e++){var d=c[e],f=document.createElement("div");f.className=" ";f.innerHTML=d[1];akeeba.System.data.set("domain",d[0]);document.getElementById("backup-steps").appendChild(f);d[0]==a?(b="akeeba-label--grey",d="akeeba-label--teal"):d=b;f.className+=" "+d}};
akeeba.Backup.start=function(){try{console.log("Starting backup"),console.log(data)}catch(c){}if(window.AVGRUN){try{console.warn("AVG Antivirus with Link Checker detected. The backup WILL fail!")}catch(c){}if(!confirm(akeeba.System.Text._("SOLO_BACKUP_AVGWARNING")))return}try{null!=akeeba.Backup.commentEditorSave&&akeeba.Backup.commentEditorSave()}catch(c){document.getElementById("comment").value=""}document.getElementById("backup-setup").style.display="none";document.getElementById("backup-progress-pane").style.display=
"block";akeeba.System.getOptions("akeeba.Backup.hasAngieKey",!1)&&(document.getElementById("angie-password-warning").style.display="block");var a=new Date;akeeba.System.notification.notify(akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_BACKUPSTARTED")+" "+a.toLocaleString());akeeba.Backup.renderBackupSteps("");a=akeeba.System.getOptions("akeeba.Backup.maxExecutionTime",14);var b=akeeba.System.getOptions("akeeba.Backup.runtimeBias",75);akeeba.Backup.startTimeoutBar(a,b);akeeba.Backup.tag=akeeba.Backup.srpInfo.tag;
a={ajax:"start",description:document.getElementById("backup-description").value,comment:document.getElementById("comment").value};a=array_merge(a,akeeba.Backup.srpInfo);akeeba.System.doAjax(a,akeeba.Backup.onStep,akeeba.Backup.onError,!1)};
...
akeeba.Backup.onError=function(a){if(akeeba.System.getOptions("akeeba.Backup.resume.enabled",!0)){var b=akeeba.System.getOptions("akeeba.Backup.resume.maxRetries");if(akeeba.Backup.resume.retry>=b)akeeba.Backup.endWithError(a);else{akeeba.Backup.resume.retry++;akeeba.Backup.resetRetryTimeoutBar();b=akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_BACKUPHALT_DESC");var c=akeeba.System.getOptions("akeeba.Backup.resume.timeout",10);b=b.replace("%d",c.toFixed(0));akeeba.System.notification.notify(akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_BACKUPHALT"),
...
akeeba.Backup.endWithError=function(a){akeeba.Backup.resetTimeoutBar();try{Piecon.reset()}catch(f){}var b=!1;document.getElementById("backup-progress-pane").style.display="none";document.getElementById("backup-warnings-panel").style.display="none";document.getElementById("retry-panel").style.display="none";var c=akeeba.System.getOptions("akeeba.Backup.URLs.LogURL"),e=akeeba.System.getOptions("akeeba.Backup.URLs.AliceURL");c=c+"&tag="+akeeba.Backup.tag;var d=e+"&log="+akeeba.Backup.tag;akeeba.Backup.backupid&&
...
function(f){});document.getElementById("backup-error-message").textContent=a;document.getElementById("error-panel").style.display="block";b&&setTimeout(function(){window.location=d},500)};
...
akeeba.Backup.flipProfile=function(){document.getElementById("flipDescription").value=document.getElementById("backup-description").value;document.getElementById("flipComment").value=document.getElementById("comment").value;document.forms.profileForm.submit()};
...
akeeba.Backup.cancelResume();return!1});akeeba.System.addEventListener("comAkeebaBackupResume","click",function(a){a.preventDefault();akeeba.Backup.resumeBackup();return!1});akeeba.System.addEventListener("profileId","change",akeeba.Backup.flipProfile);setTimeout(function(){akeeba.System.getOptions("akeeba.Backup.autoStart",!1)&&akeeba.Backup.start()},500);akeeba.System.notification.askPermission()}); //# sourceMappingURL=backup.min.map
